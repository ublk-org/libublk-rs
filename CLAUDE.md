...
- **io threads per queue** (`UblkCtrlBuilder::io_threads_per_queue(n)`,
  ...
  interleaved: 1.04M, all ~85%). `UblkFlags::UBLK_DEV_F_SEQ_TAG_PARTITION`
  (loop `-S`) opts into contiguous blocks anyway, as an experiment knob:
  measured 2026-08-29 on tomsrv it was slower for one submitter even with
  the queue full (d128 T4 QD128: 893K vs 1.03M — a submitted batch gets
