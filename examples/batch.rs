...
thread_local! {
    static BATCH_CONTEXT: RefCell<Option<u32>> = RefCell::new(None);
}
...
fn run_batch_coordination(
    ...
    // Check if we already have a batch context
    let needs_new_batch = BATCH_CONTEXT.with(|c| c.borrow().is_none());

    if needs_new_batch {
        // Phase 1: Before running tasks, create batch coordinator
        let context_id = batch_state.borrow_mut().create_coordinator();
        BATCH_CONTEXT.with(|c| *c.borrow_mut() = Some(context_id));
        ...
        if !has_registered_tasks {
            ...
            BATCH_CONTEXT.with(|c| *c.borrow_mut() = None);
        ...
        // Just run tasks if batch is already setup
        while exe.try_tick() {}
    }
}
...
async fn handle_task_batch_coordination(
    ...
    let coordinator_opt = BATCH_CONTEXT.with(|c| {
        if let Some(context_id) = *c.borrow() {
            batch_state
                .borrow()
                .get_coordinator(context_id)
                .map(|coord| (context_id, coord.batch_id))
        } else {
            None
        }
    });
